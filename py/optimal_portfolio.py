...
def plot_portfolio_weights(labels, 
                           weights, 
                           title, 
                           xlabel, 
                           metrics=None, 
                           ...
                           max_assets=5, 
                           max_asset_per_sector=1):
    """Create a portfolio weight allocation bar chart"""
    ...
    bars = plt.bar(sorted_labels, sorted_weights)
    
    # Add percentage labels to bars
    for i, bar in enumerate(bars):
        plt.text(bar.get_x() + bar.get_width()/2., bar.get_height() + 0.002,
                f'{sorted_weights[i]*100:.2f}%',
                ha='center', va='bottom')
    ...
    if metrics:
        metrics_text = '\n'.join([
            f"{k}: {v:.2%}" if is_pct else f"{k}: {v:.{precision}f}"
            for k, v, precision, is_pct in metrics
        ])
        all_annotations.append({
            'text': metrics_text,
            'xy': (0.98, 0.98),
            'bbox': {'fc': 'white', 'ec': 'gray'}
        })
    ...
    if show_constraints:
        constraints_text = (
            "Constraints:\n"
            f"Max weight per asset: {max_allocation_weight*100:.0f}%\n"
            f"Max number of assets: {max_assets}\n"
            f"Max asset per sector: {max_asset_per_sector}"
        )
        position = 0.98 - 0.12 if metrics else 0.98
        all_annotations.append({
            'text': constraints_text,
            'xy': (0.98, position),
            'bbox': {'fc': 'lightyellow', 'ec': 'orange'}
        })
    ...
    for anno in all_annotations:
        plt.annotate(
            ...
            ha='right',
            va='top',
            bbox=dict(boxstyle="round,pad=0.3", **anno['bbox'], alpha=0.8)
        )
